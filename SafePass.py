...
def get_pass(kind='pass', reenter=False):
    ...
    if os.path.exists(f'{os.path.dirname(__file__)}/{kind}') and not reenter:
        with open(f'{os.path.dirname(__file__)}/{kind}', 'r') as file:
        ...
        with open(f'{os.path.dirname(__file__)}/{kind}', 'w') as file:
